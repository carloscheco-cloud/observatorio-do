...
export function ExportButton({ resource }: { resource: string }) { return <a className="button secondary" href={`${process.env.NEXT_PUBLIC_API_URL ?? "http://localhost:8000/api/v1/public"}/export?resource=${encodeURIComponent(resource)}&format=csv`}>Exportar CSV</a>; }
