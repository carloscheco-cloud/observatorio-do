...
state_branch = sa.Enum(
    "EXECUTIVE", "LEGISLATIVE", "JUDICIAL", "CONSTITUTIONAL", "OTHER",
    name="statebranch",
)
institution_type = sa.Enum(
    "PRESIDENCY", "VICE_PRESIDENCY", "MINISTRY", "VICE_MINISTRY",
    "GENERAL_DIRECTORATE", "ATTACHED_AGENCY", "AUTONOMOUS_INSTITUTION",
    "DECENTRALIZED_INSTITUTION", "SUPERINTENDENCY", "COUNCIL", "COMMISSION",
    "CABINET", "PUBLIC_COMPANY", "PROVINCIAL_GOVERNMENT",
    "TERRITORIAL_DEPENDENCY", "OTHER",
    name="institutiontype",
)
operational_status = sa.Enum(
    "ACTIVE", "INACTIVE", "IN_REORGANIZATION", "MERGED", "DISSOLVED", "UNKNOWN",
    name="operationalstatus",
)
coverage_level = sa.Enum(
    "NONE", "BASIC", "PARTIAL", "SUBSTANTIAL", "COMPLETE",
    name="coveragelevel",
)
...
def upgrade() -> None:
    ...
    op.add_column(
        "institutions",
        sa.Column("operational_status", operational_status, nullable=False, server_default="UNKNOWN"),
    )
    op.add_column(
        "institutions",
        sa.Column("coverage_level", coverage_level, nullable=False, server_default="NONE"),
    )
    op.add_column("institutions", sa.Column("official_website", sa.String(length=500), nullable=True))
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["child_institution_id"], ["institutions.id"], ondelete="RESTRICT"
        ),
...
def downgrade() -> None:
    ...
    for column in (
        "last_reviewed_at", "creation_date", "functions_summary", "official_website",
        "coverage_level", "operational_status", "institution_type", "state_branch",
        "slug", "acronym",
    ):
